...
namespace ns0
{
	internal sealed class Class37
	{
        ...
        internal void SummonAll(Class16 session)
        {
            for (int i = 0; i < this.class16_0.Length; i++)
            {
                Class16 @class = this.class16_0[i];
                if (@class != null && @class != session && @class.method_2().uint_3 != session.method_2().uint_3)
                {
                    try
                    {
                          GClass5 gClass5 = new GClass5(286u);
                          gClass5.method_13(session.method_2().Class14_0.Boolean_3);
                          gClass5.method_11(session.method_2().uint_3); // Room ID
                          @class.method_14(gClass5);
                          @class.method_8(session.method_2().string_0 + " has summoned you to them");
                    }
                    catch
                    {
                    }
