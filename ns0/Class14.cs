...
namespace ns0
{
	internal sealed class Class14
	{
        ...
        public void sayall(string message)
        {
            ...
            {
                Class33 class4 = this.class33_0[i];
                Class16 session = class4.method_16();
                if (class4 != null)
                {
                    class4.method_1(session, message.Substring(7), false);
                }
            }
        }
        public void enableall(int Effect)
        {
            for (int i = 0; i < this.class33_0.Length; i++)
            {
                Class33 class4 = this.class33_0[i];
                Class16 session = class4.method_16();
                int int_ = Effect;
                session.method_2().method_24().method_2(int_, true);
                Phoenix.smethod_15().method_2().method_31(session, session.method_2().string_0.ToLower(), string_0);
            }
        }
